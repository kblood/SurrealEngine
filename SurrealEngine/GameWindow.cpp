...
GameWindow::GameWindow(GameWindowHost* windowHost, RenderAPI renderAPI, VulkanXRSession* xrSession) : Widget(nullptr, WidgetType::Window, renderAPI), windowHost(windowHost)
{
	SetWindowIcon({
	...
	device = RenderDevice::Create(this, renderAPI, xrSession);
...
std::unique_ptr<GameWindow> GameWindow::Create(GameWindowHost* windowHost, VulkanXRSession* xrSession)
{
	RenderAPI api;
	...
	return std::make_unique<GameWindow>(windowHost, api, xrSession);
}
