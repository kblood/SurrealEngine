...
bool DeusExTextTokenizer::Next(const std::string& source, size_t& position, const std::string& playerName, const std::string& playerFirstName, DeusExTextToken& token)
{
	token = {};
	...
	const std::string contents = source.substr(tagStart + 1, tagEnd - tagStart - 1);
	position = tagEnd + 1;
	...
	{
		const size_t equals = contents.find('=');
	...
	{
		token.Tag = DeusExTextTags::TT_Label;
		token.Name = ParseName(payload);
	}
	else if (tagName == "/<")
	{
...
void DeusExTextTokenizer::SkipWhitespace(const std::string& source, size_t& position)
{
	while (position < source.size() && (source[position] == ' ' || source[position] == '\t' || source[position] == '\r' || source[position] == '\n'))
		position++;
}

void DeusExTextTokenizer::Trim(std::string& text)
{
	size_t first = 0;
...
std::string DeusExTextTokenizer::ToUpper(std::string text)
{
	std::transform(text.begin(), text.end(), text.begin(), [](unsigned char c) { return static_cast<char>(std::toupper(c)); });
	return text;
}

bool DeusExTextTokenizer::EqualsNoCase(const std::string& left, const std::string& right)
{
	return ToUpper(left) == ToUpper(right);
}

size_t DeusExTextTokenizer::FindNoCase(const std::string& source, const std::string& text, size_t position)
{
...
std::string DeusExTextTokenizer::ParseName(const std::string& payload)
{
...
DeusExTextTokenColor DeusExTextTokenizer::ParseColor(const std::string& payload)
{
	DeusExTextTokenColor color;
	size_t position = 0;
	const std::string red = ReadField(payload, position);
	const std::string green = ReadField(payload, position);
	const std::string blue = ReadField(payload, position);
...
void DeusExTextTokenizer::ParseFile(const std::string& payload, DeusExTextToken& token)
{
	size_t position = 0;
...
void DeusExTextTokenizer::ParseEmail(const std::string& payload, DeusExTextToken& token)
{
	size_t position = 0;
...
std::string DeusExTextTokenizer::ReadField(const std::string& payload, size_t& position)
{
	if (position > payload.size())
		return {};
	const size_t separator = payload.find(',', position);
	const size_t end = separator == std::string::npos ? payload.size() : separator;
	std::string field = payload.substr(position, end - position);
	Trim(field);
	position = separator == std::string::npos ? payload.size() + 1 : separator + 1;
	return field;
}

void DeusExTextTokenizer::ConsumeBlock(const std::string& source, size_t& position, const std::string& closingTag)
{
	const size_t end = FindNoCase(source, closingTag, position);
	position = end == std::string::npos ? source.size() : end + closingTag.size();
}
...
int ReadDeusExTextPage(const std::string& text, int page, std::string& output)
{
	...
	const size_t start = static_cast<size_t>(page) * pageSize;
	if (start >= text.size())
	{
		output.clear();
		return 0;
	...
	const size_t count = std::min(pageSize, text.size() - start);
