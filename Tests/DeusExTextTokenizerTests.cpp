...
static DeusExTextToken Next(const std::string& source, size_t& position, const std::string& message)
{
	DeusExTextToken token;
	Check(DeusExTextTokenizer::Next(source, position, "JC Denton", "JC", token), message + " should produce a token");
	return token;
}
...
static void TestStockDocument()
{
	...
	size_t position = 0;
	DeusExTextToken token = Next(source, position, "default color");
	...
	Check(!DeusExTextTokenizer::Next(source, position, "", "", token), "EOF does not produce a token");
}

static void TestMetadata()
{
	size_t position = 0;
	DeusExTextToken token = Next("<FILE=01_Bulletin01, Terrorism -- Crime or Conscience?>", position, "file");
	...
	Check(token.Tag == DeusExTextTags::TT_Email, "EMAIL tag is distinct from FILE");
...
static void TestBlocksAndSubstitutions()
{
	...
	size_t position = 0;
	DeusExTextToken token = Next(source, position, "comment");
	...
	position = 0;
	const std::string goalSource = "<GOAL=PrimaryGoal>ignored</GOAL><P>";
	token = Next(goalSource, position, "goal");
	Check(token.Tag == DeusExTextTags::TT_Goal && token.Name == "PrimaryGoal", "GOAL name");
	Check(Next(goalSource, position, "after goal").Tag == DeusExTextTags::TT_NewParagraph, "GOAL consumes its block");
}
...
static void TestCompleteTokenTable()
{
	...
	{
		size_t position = 0;
		const DeusExTextToken token = Next(item.Source, position, item.Source);
		Check(token.Tag == item.Tag, std::string("token table entry ") + item.Source);
	}

	size_t position = 0;
	Check(Next("<G=Logo>", position, "graphic name").Name == "Logo", "graphic name payload");
	position = 0;
	Check(Next("<F=FontMenuSmall>", position, "font name").Name == "FontMenuSmall", "font name payload");
	position = 0;
	Check(Next("<L=Start>", position, "label name").Name == "Start", "label name payload");
	position = 0;
	const std::string bracketsSource = "</<>text</>";
	Check(Next(bracketsSource, position, "open bracket").Text == "<", "open bracket substitution");
	Check(Next(bracketsSource, position, "bracket body").Text == "text", "bracket body");
	Check(Next(bracketsSource, position, "close bracket").Text == ">", "close bracket substitution");
}
...
static void TestTextPaging()
{
	...
	std::string page;
	Check(ReadDeusExTextPage(source, 0, page) == 239 && page == source.substr(0, 239), "first 239-character page");
	Check(ReadDeusExTextPage(source, 1, page) == 239 && page == source.substr(239, 239), "second 239-character page");
	Check(ReadDeusExTextPage(source, 2, page) == 22 && page == source.substr(478), "short final page");
	Check(ReadDeusExTextPage(source, 3, page) == 0 && page.empty(), "page after EOF");
	Check(ReadDeusExTextPage(std::string(239, 'x'), 0, page) == 239, "exact page size");
	Check(ReadDeusExTextPage(std::string(239, 'x'), 1, page) == 0 && page.empty(), "exact page boundary EOF");
	Check(ReadDeusExTextPage("", 0, page) == 0 && page.empty(), "empty text page");
	Check(ReadDeusExTextPage(source, -1, page) == 0 && page.empty(), "negative page");
}
