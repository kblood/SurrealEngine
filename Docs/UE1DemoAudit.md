...
| Unreal Special Edition / OEM demo 205 | Passes requested and automatic synthetic import as `unreal-demo-205`. Its real contract uses `UnrealI.u`, `UnrealIOrder.u`, and `Default.ini`; it does not pretend to be Unreal Gold. Mutable `SE-Unreal.ini` is allowed. | Passes exact SHA-1 `b851dcc69c4f773252c0498bd12756d90bcb59c2` as `UNREAL_205_DEMO`, version `205`. | Detection passes. Focused compatibility fixes handle v61's reflected `Parent` container name and its pre-`StrProperty` `StructProperty(DynamicString)` representation, advancing beyond both the original `TextBuffer.Outer` and `Player.TravelItems` failures. The bounded scan now stops at the unrelated missing package dependency `UPak`; no menu or map boot was attempted. | Determine whether `UPak` is absent by design from this incomplete OEM distribution or should be supplied by another lawful original artifact. Keep that content/dependency decision separate from serialization compatibility, then retry `Vortex2`. Do not use the third-party no-CD/fix archive. | **Unclear.** “Shareware” is historical evidence, not an explicit license in this archive. No authoritative current publisher grant for this exact repack was located. Require local import unless permission and an authoritative original artifact are obtained. |
...
4. Keep each Unreal 205 property-layout rule isolated and covered by a focused
