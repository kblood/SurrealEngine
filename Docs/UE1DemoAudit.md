...
4. Keep each Unreal 205 property-layout rule isolated and covered by a focused
   regression test. The v61 `Parent`/`Outer` reflection rule is independent of
   the remaining `TravelItems` aggregate-size failure; do not weaken bounds or
   property lookup globally to force a boot.
