...
The engine-side menu/HUD/cinematic/loading policy is no longer the missing part: native OpenXR and WebXR now use the same descriptor builder, viewer anchoring, composition ordering, both-hand pointer router, exact contact, click-edge semantics, mouse fallback, and cleanup behavior. The remaining blocker is specifically Vulkan target production and OpenXR composition. `VulkanRenderDevice` currently accepts only stereo target slot 1, renders the complete side-by-side desktop composition into the window image, and blits its halves into the acquired eye images during `Flush`. It cannot bind slots 2-5 as independent single-surface render targets, preserve their contents, or sample/submit them as depth-independent projection overlays or `XrCompositionLayerQuad` layers. Therefore no production `OpenXRUICompositionSink` is connected yet; doing so before those operations exist would expose interactive but invisible surfaces.

The next renderer topic is narrowly defined: allocate persistent Vulkan images for slots 2-5, let `BeginPresentationLayer` select and clear each target, make the images available to an OpenXR compositor, submit the shared replay order with menu last and world depth ignored, and release all images on session exit/re-entry. It must not reimplement descriptor, anchoring, input, startup, or menu policy. Haptic transport exists, but no weapon, UI, or game-profile policy submits feedback yet.
