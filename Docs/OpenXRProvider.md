...
Native blocking AVI playback still uses its older private draw loop rather than `Engine::RunOneFrame`, so it does not yet drive an OpenXR frame or submit the cinematic quad while that loop is active. Loading visibility likewise has no native producer yet. Their swapchains and composition roles are implemented, but those two producers must be moved onto the balanced XR frame scheduler before they are visible in a headset. Runtime validation still needs a real Vulkan OpenXR runtime/HMD, especially for alpha, image orientation, runtime-specific swapchain formats, session-loss timing, and compositor layer limits. Haptic transport exists, but no weapon, UI, or game-profile policy submits feedback yet.
