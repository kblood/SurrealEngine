...
This topic extracts the native OpenXR/Vulkan presentation path behind the shared frame, view-family, and presentation contracts. It does not contain UT99-specific weapon behavior, controller actions, menu interaction, launcher game selection, or a second engine loop.
...
- `Engine::RunOneFrame` still advances once, renders once through a selected `ViewFamily`, finishes once, and balances any active OpenXR frame around only the render phase.
...
None of the later controller, weapon, menu, fixed render-size, or UT99 gameplay changes were transplanted.
