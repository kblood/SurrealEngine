...
The next visual-only position tranche extends the existing re-entrant
`Frame::CallScopeHook` without intercepting general actor rendering. During the
current local weapon's exact global `RenderOverlays` scope, the engine freezes
the selected controller index and the already selected aim-preferred (grip
fallback) full-basis presentation rotation. Only the exact nested native
`Engine.Canvas.DrawActor` call is eligible, and only when its receiver is the
engine canvas, its three-argument signature is intact, its first argument is
that same weapon, and local pawn/weapon/owner relationships still match.

Immediately around that native draw, the weapon's visual location becomes the
tracked dominant grip world position plus `forward * offset.x + right *
offset.y + up * offset.z`; its rotation remains the frozen aim-preferred
presentation rotation. Actor location and rotation are restored exactly in
LIFO order before UnrealScript continues. Stock `RenderOverlays`, animation,
mesh selection, and muzzle-flash script therefore still run unchanged. The
implementation does not touch `CalcDrawOffset`, `FireOffset`, projectile or
trace origins, collision, or simulation state.

Grip metadata uses immutable schema v1 rows keyed by both package and class.
The production table intentionally contains no guessed values: every weapon
currently takes the explicit zero-offset fallback, placing its model origin at
the grip position. Deterministic tests cover exact-call rejection, same-class
package collisions, full-basis offset composition, zero-offset equality with
the grip position, non-finite rejection, aim/grip rotation separation, and
nested byte-exact restoration. Browser diagnostics expose schema version,
scope/restore, calibrated/fallback/rejection counts, last offset, and last
visual position.

This closes only the safe placement seam. It does not claim that a stock mesh
origin is a comfortable grip, that a barrel or muzzle flash aligns with the
controller ray, or that left-handed mirroring, weapon scale, dual Enforcers,
two-hand support, and special/automatic weapons are correct. Hardware-qualified
package/class offsets, WebGPU culling inspection, and physical Quest alignment
remain required before enabling nonzero production rows or changing any firing
origin.

The loaded single-weapon browser gate is now complete. The explicit diagnostic
`Surreal_RunWebXRLoadedWeaponFixture` has no startup or production caller. It
uses the loaded retail package's `GameInfo.GiveWeapon` function to acquire the
exact `Botpack.ShockRifle` when absent, walks the real pawn inventory chain,
then selects it via `PendingWeapon` plus the pawn's stock `ChangedWeapon`
function. It never directly constructs a weapon or assigns `Pawn.Weapon`.
Repeat invocation is safe and observable through delta-friendly
attempt/give/select/inventory/equipped/succeeded exports; smoke proves the
second call reuses inventory, skips `GiveWeapon`, and still exercises the
benign stock `ChangedWeapon` path.

Validation on 2026-07-22 passes Python compilation and `tabnanny`, the native
Debug target, data-backed and no-data Emscripten targets, and the full
`--experimental-webgpu-xr` Playwright suite. The loaded packed frame reports
`weaponCalls=2`, two exact native `Engine.Canvas.DrawActor` visual scopes, two
restores, two zero-offset fallbacks, zero calibrated offsets, and zero rejected
transforms. Its finite last visual position
`[2343.1387, -7.3820, -433.5940]` equals the tracked dominant grip within
`0.001` UU; expected/completed weapon eye passes remain `2/2`, and the packed
target contains 201 draws with distinct eyes. A subsequent pose/input reset
and frame clears sources/dominant selection without adding scopes or
rejections. All existing input, haptic, HUD/menu, audio, visibility,
repeat-entry, and device-loss gates also pass. Automatic/special weapons,
...
- **Implemented in commit `dd968728`:** a schema-v1 qualification harness uses
  the actual mutable OPFS/IndexedDB and importer OPFS APIs only in unique
  synthetic namespaces inside an owned disposable browser profile. Chrome 150
  and installed Brave 150 each passed 6/6: orderly full-browser restart,
  mutable/importer separation, renderer crash, exact process-tree kill after
  partial mutable and importer OPFS generations plus an in-flight IndexedDB
  replacement, six corruption/future-schema fail-closed checks, and explicit
...
The importer and mutable-store qualification still uses synthetic fake files.
It now proves controlled desktop browser restart, renderer crash, process-tree
kill recovery, corruption refusal, namespace isolation, and explicit site-data
clearing; it does not prove a complete user-owned UT99 import, real Quest
Browser folder permissions or OS kill, storage-pressure eviction, full-size
quota/copy/thermal behavior, custom save paths/names, an implemented schema
migration, or clean-profile launch into a playable map. Those remain explicit
M10 acceptance gates; no commercial content was imported, committed, embedded,
or requested here. See `WEBXR_STORAGE_ROBUSTNESS.md`.
...
9. **In progress:** the local schema-v1 OPFS/IndexedDB importer passes 13/13
   ...
   store against physical Quest restart, pressure eviction, and OS termination;
