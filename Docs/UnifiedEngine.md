...
At physical-candidate commit `802cfa62` (runtime code through `078a6d2f`), the
main product-only additions beyond the foundation topics are:
...
| WebXR UI and controllers | Implemented, experimental; partial hardware evidence | Candidate `173bf623` rendered tracked blue/red procedural controller proxies. On `802cfa62`, controller aim tracked and menu pointers could point and click, but the menu was horizontally mirrored. The shared geometry now drives WebXR and an optional native OpenXR source-alpha projection overlay: both tracked hands remain visible while UI is active, while only the configurable dominant hand owns the beam and exact hit marker. Native runtimes with five compositor layers retain projection plus four topmost quads; a sixth layer enables the visual overlay. Provider orientation, scale, line visibility, convergence, latency, and comfort still require physical requalification. |
| Semantic XR gameplay input | Implemented, experimental; prior hardware-observed input failure | WebXR and OpenXR use the provider-neutral `XRInputAdapter`; right-dominant Select maps to Fire, the other Select to AltFire, sticks map to movement/turning, and independent sources preserve keyboard/mouse fallback. The post-`802cfa62` integration canonicalizes a pressed WebXR trigger, routes controller back/menu through real Escape edges, restores UE1's 7000-scale movement axes, and applies native head-relative locomotion before simulation. Bounded native diagnostics now report focus, semantic action availability, trigger/menu edges, and nonzero stick samples without poses or raw values. These fixes pass automated gates but remain unqualified on Quest 3/VDXR; snap-turn settings, remapping, handedness UI, and comfort remain follow-ups. |
...
attempts and waiting to remain immersive. Head look, controller aim, and menu
point/click responded. The controller trigger did not satisfy **Press Fire**
although mouse left did, thumbsticks did not move the player, the menu was
horizontally mirrored, and the world remained distorted. The local privacy-safe
report is pending and the page sends no remote telemetry, so mode/stage/frame,
input, projection, reference-space, and pose-age fields were not captured. This
is a hardware-observed failed qualification and the candidate remains non-final.

For an A/B that isolates shared engine/XR behavior from WebXR, a data-free
native Windows x64 OpenXR package was built from exact `802cfa62` and passed all
34 registered tests. It is at
`C:\Devstuff\QuestGames\release-candidates\SurrealEngine-Native-OpenXR-802cfa62-VDXR-test`.
With Quest 3 connected through Virtual Desktop and VDXR active, launch its
...
bind OpenXR. Native `--render=vulkan` is not applied at this commit because the
command-line renderer override is Emscripten-only. Repeat identical head, aim,
menu click, gameplay trigger, thumbstick, world-orientation, and menu-handedness
...
1. Validate both direct `XRGPUBinding` (where exposed) and automatic/forced
   ...
   Preserve the local report from candidate `802cfa62`, then run the exact
   native OpenXR/Vulkan package as an A/B with the same game and input sequence.
