...
def _validate_run(run: Path, artifacts: dict[str, Artifact]) -> None:
    ...
    if manifest_schema.endswith("v2"):
        ...
        if "targetless_move_to_timeout_enabled" in manifest:
            if not isinstance(manifest["targetless_move_to_timeout_enabled"], bool):
                raise ComparisonError(
        ...
        if "native_path_commit_observer_enabled" in manifest:
            if not isinstance(manifest["native_path_commit_observer_enabled"], bool):
                raise ComparisonError(
        ...
        if "direct_reach_command_observer_enabled" in manifest:
            if not isinstance(manifest["direct_reach_command_observer_enabled"], bool):
                raise ComparisonError(
                    f"{run}/manifest.json.direct_reach_command_observer_enabled: expected a boolean")
            config_fields += ("direct_reach_command_observer_enabled",)
    ...
    if "targetless_move_to_timeout_enabled" in config_fields and not isinstance(
    ...
    if "native_path_commit_observer_enabled" in config_fields and not isinstance(
    ...
    if "direct_reach_command_observer_enabled" in config_fields and not isinstance(
        ...
        raise ComparisonError(
            f"{run}/summary.json.config.direct_reach_command_observer_enabled: expected a boolean")
